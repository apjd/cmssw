...
namespace trajectoryStateTransform {
  ...
  FreeTrajectoryState initialFreeStateTTrack(const TTTrack< Ref_Phase2TrackerDigi_ >& tk, const MagneticField* field, bool withErr) {
    ...
    LogDebug("L1Seeds") << "L1 TRACK! " << std::endl;
    LogDebug("L1Seeds") << "speed of light " << speedOfLightConverted << std::endl;
    LogDebug("L1Seeds") << "trk_signedPt : " << trk_signedPt << std::endl;
    LogDebug("L1Seeds") << "trk_signedPt mom: " << gmom.transverse() << std::endl;
    LogDebug("L1Seeds") << "Q: " << trk_signedPt/gmom.transverse() << std::endl;
